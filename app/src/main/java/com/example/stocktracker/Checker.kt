...
private object WebChecker {
                ...
                override fun onPageFinished(view: WebView, loadedUrl: String) {
                    ...
                    fun inspect() {
                        ...
                        view.evaluateJavascript(js) { raw ->
                            ...
                            val r = if (isTarget) {
                                parseRenderedTarget(title, body, targetPrice, targetPrices, stockSignals)
                            } else if (isAmazon) {
    ...
    private fun parseRenderedTarget(
        ...
        // Prefer Target's product-price DOM nodes. The page contains many other
        // prices in recommendation cards, so a generic first-$ regex is unsafe.
        ...
        // Target's rendered accessibility/text representation currently places the
        // product price immediately before the product availability message. Use the
        // product-title region as a secondary fallback, never recommendation cards.
        if (price == null) {
            ...
            if (start >= 0) {
                ...
                val end = endCandidates.minOrNull() ?: minOf(body.length, start + 2500)
                price = moneyFromText(body.substring(start, end))
            }
        }

        val combined = clean((stockSignals + listOf(body)).joinToString(" ")).lowercase()
        val stock = when {
            "out of stock" in combined || "sold out" in combined || "currently unavailable" in combined -> false
            Regex("\\b(add to cart|ship it|deliver it|pickup|pick it up|same day delivery)\\b")

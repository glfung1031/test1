...
object Checker {
    ...
    private fun walkJsonObjects(value: Any?, visitor: (JSONObject) -> Unit) {
        when (value) {
            is JSONObject -> {
                visitor(value)
                val keys = value.keys()
...
private object WebChecker {
    ...
    private fun parseRenderedTarget(
        ...
        // Price: use Target's explicit product-price nodes first. Target renders
        // recommendation prices elsewhere on the page, so generic page-wide $ regexes
        // are deliberately only a last resort.
        ...
        // Target's own page JSON is more reliable than visible text for price. The
        // rendered page may contain __TGT_DATA__/__NEXT_DATA__ even when the price DOM
        // selector changes.
        if (price == null) {
            for (blob in targetData) {
                try {
                    val obj = JSONObject(blob)
                    walkJsonObjects(obj) { o ->
                        for (key in listOf("current_retail", "formatted_current_price", "reg_retail")) {
                            if (price == null && o.has(key)) {
                                val raw = o.opt(key)?.toString()?.replace("$", "")?.replace(",", "")
                                raw?.toDoubleOrNull()?.let { price = it }
                            }
                        }
                    }
                } catch (_: Exception) {}
        ...
        // Final text fallback: restrict to the product's own section.
        if (price == null) {
            val titlePos = if (title.isNotBlank()) body.indexOf(title, ignoreCase = true) else -1
            val start = if (titlePos >= 0) titlePos else body.indexOf("New at Target", ignoreCase = true)
            if (start >= 0) {
                val endCandidates = listOf(
                    body.indexOf("About this item", start, ignoreCase = true),
                    body.indexOf("Shipping & Returns", start, ignoreCase = true)
                ).filter { it > start }
                val end = endCandidates.minOrNull() ?: minOf(body.length, start + 3000)
                price = moneyFromText(body.substring(start, end))
            }
        }

        // Keep the previously working Target stock logic: inspect both the dedicated
        // fulfillment nodes and the product-page text. Do not let recommendation cards
        // determine stock.
        val productText = run {
            val titlePos = if (title.isNotBlank()) body.indexOf(title, ignoreCase = true) else -1
            if (titlePos >= 0) body.substring(titlePos, minOf(body.length, titlePos + 5000)) else body
        }
        val combined = clean((stockSignals + listOf(productText)).joinToString(" ")).lowercase()
        val stock = when {
            "out of stock" in combined || "sold out" in combined || "currently unavailable" in combined -> false
            Regex("\\b(add to cart|ship it|deliver it|pickup|pick it up|same day delivery|in stock)\\b")
                .containsMatchIn(combined) -> true
            else -> null
        }

        // If visible text did not expose stock, use the embedded Target JSON as a
        // fallback, matching the stock parser from the prior working version.
        var jsonStock = stock
        for (blob in targetData) {
            ...
            try {
                ...
                walkJsonObjects(obj) { o ->
                    ...
                    if (o.has("is_out_of_stock") && jsonStock == null)
                        jsonStock = !o.optBoolean("is_out_of_stock")
                }
            } catch (_: Exception) {}
        }

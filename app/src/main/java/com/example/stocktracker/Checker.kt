...
object Checker {
    ...
    private fun walk(v: Any?, f: (JSONObject) -> Unit) {
        when (v) {
            is JSONObject -> { f(v); v.keys().forEach { walk(v.opt(it), f) } }
    ...
    private fun jsonLdOffers(html: String): Pair<Double?, Boolean?> {
        ...
        Regex("<script[^>]*application/ld\\+json[^>]*>(.*?)</script>", RegexOption.DOT_MATCHES_ALL)
            .findAll(html).forEach { m ->
                try {
                    walk(JSONTokener(m.groupValues[1].trim()).nextValue()) { o ->
                        val offers = o.opt("offers") ?: return@walk
    ...
    private fun parseAmazon(html: String): Result {
    ...
    private fun parseTarget(html: String): Result {
        var price: Double? = null; var stock: Boolean? = null
...
private object WebChecker {
        ...
        main.post {
            ...
            web.webViewClient = object : WebViewClient() {
                override fun onPageFinished(view: WebView, loadedUrl: String) {
                    // Let React hydrate and any async product data render.
                    main.postDelayed({
                        val js = """
                            (function() {
                        ...
                        view.evaluateJavascript(js) { raw ->
                            ...
                            result = if (finalUrl.lowercase().contains("target.com"))
                                parseRenderedTarget(title, body, html)
                            else if (finalUrl.lowercase().contains("amazon.") ||
                                finalUrl.lowercase().contains("amzn."))
                                parseRenderedAmazon(body, html)
                            else
    ...
    private fun parseRenderedTarget(titleRaw: String, bodyRaw: String, html: String): Result {
        val body = clean(bodyRaw)
        val title = clean(titleRaw)
        // Target's product area appears before "About this item". Restrict price
        // and availability detection to that area so recommendation cards do not
        // contaminate the result.
        val start = if (title.isNotBlank()) maxOf(0, body.indexOf(title)) else 0
        val end = listOf(
            body.indexOf("About this item", start),
            body.indexOf("Shipping & Returns", start)
        ).filter { it >= 0 }.minOrNull() ?: minOf(body.length, start + 5000)
        val section = body.substring(start, end)
        val price = Regex("\\$(\\d{1,4}(?:,\\d{3})*(?:\\.\\d{2})?)")
            .find(section)?.groupValues?.get(1)?.replace(",", "")?.toDoubleOrNull()

        val lower = section.lowercase()
        val stock = when {
            "out of stock" in lower || "sold out" in lower || "currently unavailable" in lower -> false
            Regex("\\b(add to cart|ship it|deliver it|pickup|pick it up|same day delivery)\\b", RegexOption.IGNORE_CASE)
                .containsMatchIn(section) -> true
            else -> null
        }
        if (price == null && stock == null) {
            val (p, s) = parseTarget(html)
            if (p != null || s != null) return Result(p, s)
            return Result(null, null, if (lower.contains("captcha") || lower.contains("verify"))
                "Target presented a verification page" else "Target rendered, but price/stock could not be found")
        }
        return Result(price, stock)
    }

    private fun parseRenderedAmazon(bodyRaw: String, html: String): Result {
        val body = clean(bodyRaw)
        val lower = body.lowercase()
        val price = Regex("\\$(\\d{1,4}(?:,\\d{3})*(?:\\.\\d{2})?)")
            .find(body)?.groupValues?.get(1)?.replace(",", "")?.toDoubleOrNull()
        val stock = when {
            "currently unavailable" in lower || "out of stock" in lower -> false
            "in stock" in lower || "add to cart" in lower -> true
            else -> null
        }
        if (price == null && stock == null) {
            val parsed = parseAmazon(html)
            if (parsed.price != null || parsed.inStock != null) return parsed
            return Result(null, null, if ("robot check" in lower || "captcha" in lower)

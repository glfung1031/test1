...
private object WebChecker {
                    ...
                    fun inspect() {
                        ...
                        val js = """
                            (function() {
                              ...
                              const h1 = first(['h1']);
                              // Target's useful product information is contained in the
                              // rendered product-detail component. Do not send the entire
                              // page back through the WebView bridge: recommendation cards
                              // contain unrelated prices and stock messages and can also
                              // make the JS result unnecessarily large.
                              ...
                              if (u.includes('target.com')) {
                                ...
                                if (h) {
                                  ...
                                  for (let i = 0; i < 10 && node; i++, node = node.parentElement) {
                                    const t = txt(node);
                                    if (t && t.length >= h1.length && t.length <= 3500 && /\$\s*[0-9]/.test(t) && /(in stock|out of stock|sold out|add to cart|unavailable)/i.test(t)) {
                                      best = t;
                                      // The first/shortest matching ancestor is normally
                                      // the product-detail container.
                                      break;
                                    }
                                  }
                                  targetProductText = best || txt(h.parentElement);
                                }
                              }
                              return JSON.stringify({
                                title:h1,
                                body:document.body?.innerText || '',
                                targetProductText:targetProductText,
                                url:location.href,
                        ...
                        view.evaluateJavascript(js) { raw ->
                            ...
                            val targetProductText = payload.optString("targetProductText")
                            ...
                            val r = if (isTarget) {
                                parseRenderedTarget(title, body, targetProductText, targetPrice, targetPrices, stockSignals, targetData, jsonLd, metaPrice)
                            } else if (isAmazon) {
    ...
    private fun moneyFromText(text: String): Double? {
        ...
        return Regex("\\$\\s*([0-9]+(?:,[0-9]{3})*(?:\\.[0-9]{2})?)")
    ...
    private fun parseRenderedTarget(
        ...
        // Use the product-detail text captured directly from the rendered Target
        // component. This avoids both recommendation-card false positives and huge
        // WebView bridge payloads. Fall back to the old H1-based extraction only if
        // the component could not be located.
        val productText = if (renderedProductRaw.isNotBlank()) clean(renderedProductRaw) else run {
            val titlePos = if (title.isNotBlank()) body.indexOf(title, ignoreCase = true) else -1
            val productStart = if (titlePos >= 0) titlePos else -1
            if (productStart >= 0) {
        ...
        // The rendered product-detail component is authoritative. In particular,
        // never let generic page-level shipping/recommendation widgets override an
        // explicit "Out of Stock" for the requested product.
        val combined = clean(productText).lowercase()
        val stock = when {
